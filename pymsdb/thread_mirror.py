...
from .stackframe_mirror import StackFrameMirror
...
class ThreadMirror:
    ...
    def __str__(self):
        return "Thread Mirror, name = {0}".format(self.get_name())
    ...
    def get_name(self):
        if self._name is None:
            ...
            real_name = BufferStream(answer.data).get_string()
            if real_name == "":
    ...
    def get_is_from_threadpool(self):
        if self._is_from_threadpool is None:
            ...
            self._is_from_threadpool = (
                BufferStream(answer.data).get_byte() == 1)
    ...
    def get_state(self):
        ...
        state = BufferStream(answer.data).get_int()
        return state
