...
from .field_mirror import FieldMirror
from .property_mirror import PropertyMirror
...
class TypeMirror:
    ...
    def __str__(self):
        return "Type Mirror, fullname = {0}".format(self.get_fullname())
    ...
    def get_methods(self):
        if self._methods_ids is None:
            ...
            self._methods_ids = (
                BufferStream(answer.data).get_array(sdbtypes.decode_int))
    ...
    def get_source_files(self):
        if self._source_files is None:
            ...
            self._source_files = (
                BufferStream(answer.data).get_array(sdbtypes.decode_string))
    ...
    def get_fields(self):
        if self._fields is None:
            ...
            self._fields = BufferStream(answer.data).get_array(
                decode_field_info)
    ...
    def get_static_fields_values(self, fields):
        ...
        values = BufferStream(answer.data).get_array_known_size(
    ...
    def get_properties(self):
        if self._properties is None:
            ...
            self._properties = BufferStream(answer.data).get_array(
                decode_property)
    ...
    def is_assignable_from(self, other_type):
        ...
        result = (BufferStream(answer.data).get_byte() == 1)
        return result
