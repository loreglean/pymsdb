...
from .vm_mirror import VmMirror
from .events_data import create_event_data
from .exceptions import error_code_to_exception
...
class Agent:
    ...
    def send_command(self, command_set, command_id, params=b""):
        ...
        error_code = BufferStream(event.answer.header).skip(9).get_short()
        error = error_code_to_exception(error_code)
    ...
    def enable_event(self, event_kind, suspend_policy, *modifiers):
        ...
        error_code = BufferStream(answer.header).skip(9).get_short()
        error = error_code_to_exception(error_code)
        if error is not None:
            raise error

        request_id = BufferStream(answer.data).get_int()
    ...
    def _listen(self):
        ...
        while self._is_listening:
            ...
            packet_length = BufferStream(header).get_int()
    ...
    def _process_packet(self, packet):
        packet_flag = BufferStream(packet).skip(8).get_byte()
    ...
    def _process_cmd_packet(self, packet):
        packet_stream = BufferStream(packet).skip(9)
    ...
    def _on_event_set(self, suspend_policy, events_data):
        for event_data in events_data:
            ...
            if event_data.event_kind == constants.EVENT_KIND_VM_START:
                self.vm = VmMirror(self, event_data.root_appdomain_id)
